#==============================================================================
# Make a standardized netCDF file for MADA.
...
#%% LOAD DATA
# Note: data can be downloaded at: https://www.ncei.noaa.gov/access/paleo-search/study/30612

print('=== Processing MADA ===')
data_dir = '/projects/pd_lab/data/paleoclimate_reconstructions/drought_atlases/'

# Load data
data_xarray = xr.open_dataset(data_dir+'SADA_t.nc')
var_spatial_mean = data_xarray['scpdsi'].values
...
var_global_mean = data_xarray['scpdsi'].weighted(lat_weights).mean(('longitude','latitude')).values  #TODO: Check this. Maybe do it manually
data_xarray.close()
...
# MADA only has one ensemble member, so the mean and the ensemble member are the same.
...
notes = ['MADA only has one ensemble member, so the mean and the ensemble member are the same.']
...
data_xarray_output = xr.Dataset(
    ...
    attrs={
        'dataset_name':      'South American Drought Atlas',
        'dataset_source_url':'https://www.ncei.noaa.gov/access/paleo-search/study/30612',
    },
)
...
data_xarray_output.to_netcdf(output_dir+'sada_v1_0_0_pdsi_djf.nc')
