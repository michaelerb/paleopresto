#==============================================================================
# Make a standardized netCDF file for NADA.
...
#%% LOAD DATA
# Note: data can be downloaded at: https://www.ncei.noaa.gov/access/paleo-search/study/19119

print('=== Processing NADA ===')
data_dir = '/projects/pd_lab/data/paleoclimate_reconstructions/drought_atlases/'

# Load data
data_xarray = xr.open_dataset(data_dir+'nada_hd2_cl.nc')
...
# Set all -99.999 values to nan
var_spatial_mean[var_spatial_mean == -99.999] = np.nan
...
# NADA only has one ensemble member, so the mean and the ensemble member are the same.
...
notes = ['NADA only has one ensemble member, so the mean and the ensemble member are the same.']
...
data_xarray_output = xr.Dataset(
    ...
    attrs={
        ...
        'dataset_source_url':'https://www.ncei.noaa.gov/access/paleo-search/study/19119',
    },
)
...
data_xarray_output.to_netcdf(output_dir+'nada_v1_0_0_pdsi_jja.nc')
