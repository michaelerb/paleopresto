#==============================================================================
# Make a standardized netCDF file for ANZDA.
...
#%% LOAD DATA
# Note: data can be downloaded at: https://www.ncei.noaa.gov/access/paleo-search/study/20245
# Note: The ANZDA reconstruction starts in the December of the previous year.
#   A minus 1 has been appplied the years to make ANZDA more comparable to other reconstructions in PReSto.

print('=== Processing ANZDA ===')
data_dir = '/projects/pd_lab/data/paleoclimate_reconstructions/drought_atlases/'

# Load data
data_csv   = pd.read_csv(data_dir+'anzda-recon.txt',delimiter='\t')
latlon_csv = pd.read_csv(data_dir+'anzda-pdsi-xy.txt',delimiter='\t',header=None)
years = np.array(data_csv['Year'])-1  # Minus 1 has been added to account for the fact that the ANZDA reconstruction starts in the December of the previous year.
...
lat = np.arange(-46.75,-11,.5)
lon = np.arange(136.25,178.5,.5)
...
# ANZDA only has one ensemble member, so the mean and the ensemble member are the same.
...
notes = ['ANZDA only has one ensemble member, so the mean and the ensemble member are the same.']
...
data_xarray_output = xr.Dataset(
    ...
    attrs={
        'dataset_name':      'Eastern Australia and New Zealand Drought Atlas',
        'dataset_source_url':'https://www.ncei.noaa.gov/access/paleo-search/study/20245',
    },
)
...
data_xarray_output.to_netcdf(output_dir+'anzda_v1_0_0_pdsi_djf.nc')
